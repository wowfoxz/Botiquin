...
// Tipo para las propiedades que se pasan a los elementos hijos
type DockChildProps = {
  width?: MotionValue<number>;
  isHovered?: MotionValue<number>;
};
    ...
    >
      {Children.map(children, (child) => {
        if (React.isValidElement(child)) {
          return cloneElement(child, { width, isHovered } as DockChildProps);
        }
        return child;
      })}
    </motion.div>
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-[var(--color-border)] bg-[var(--color-muted)] px-2 py-0.5 text-xs text-[var(--color-muted-foreground)]',
            className
          )}
          role='tooltip'
          style={{ x: '-50%' }}
